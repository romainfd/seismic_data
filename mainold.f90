...
num=0
open(12,file=('R98in'),status='unknown')
IOstatus=0
ie=1
il=1
...
il=il-2
close(12)
allocate(output(nevent,13))


open(10,file=('stautm98.t'),status='unknown')
do j=1,nstatot	
	read(10,'(A3)')namestatot(j)
enddo
close(10)

ie=1
open(42,file='R98in',status='unknown')
do while(ie .le. nevent)
  nstatp=0
  nstats=0
  namestatp='000'
  namestats='000'
  ...
  tcalp=0.
  tcals=0.
  do ii=1,num(ie+1)-num(ie)-1
    read(42,'(A5,A4,A21,A7,A3)')temp1,temp2,temp3,temp4,temp5
    !cas ligne P
!    	write(*,*)ii,'p',temp5
...
!	write(*,*)ii,'p',temp1(2:4),temp3     
      ...
      namestatp(nstatp)=temp1(1:3)
        ...
        tobsp(nstatp,2)=ichar(temp2(3:3))-48
...
!Indice station à la place du nom
  do i=1,nstatp
    do j=1,nstatot
      if(namestatp(i) .eq. namestatot(j)) then
        numberstatp(i)=j
 !    write(*,*)numberstatp(i)
      endif
    enddo
...
! open(110,file='res.bin',form='unformatted',status='unknown',access='direct', &
!     recl=4*ny*nx*nz)
! write(110,rec=1)res
! close(110)

! open(120,file='res2.bin',form='unformatted',status='unknown',access='direct', &
!     recl=4*ny*nx*nz)
! write(120,rec=1)res2
! close(120)
